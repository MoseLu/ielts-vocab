...
| --- | --- | --- | --- | --- | --- | --- |
| `gateway-bff` | Browser-facing gateway, auth edge, aggregation, compatibility | No long-term business-domain writes; temporary request/session orchestration only | May read downstream APIs and compatibility caches | Minimal edge-only state | [app.py](../../backend/app.py), [routes/**](../../backend/routes), [routes/middleware.py](../../backend/routes/middleware.py) | Phase 3 |
| `asr-service` | Realtime ASR, file transcription, audio-session lifecycle | Realtime session state, transcription job state, ASR temp artifacts | May read gateway-propagated user context and content hints | ASR session store and temp artifact store | [speech_service.py](../../backend/speech_service.py), [speech_socketio.py](../../backend/routes/speech_socketio.py), [asr_service.py](../../backend/services/asr_service.py) | Phase 2 |
| `tts-media-service` | TTS generation, audio cache, media batch jobs | Audio cache metadata, media job state, generated artifacts | May read catalog content and gateway user context | Media cache store and generated object/file storage | [tts.py](../../backend/routes/tts.py), [word_tts.py](../../backend/services/word_tts.py), [word_tts_service](../../backend/services/word_tts_service) | Phase 2 |
| `catalog-content-service` | Vocabulary catalog, chapters, word detail, enrichment, examples | Books, chapters, content metadata, word detail enrichment, content indexes | May expose read APIs to gateway, TTS, AI execution, learning-core | Catalog/content database and search indexes | [books_vocabulary_loader_service.py](../../backend/services/books_vocabulary_loader_service.py), [word_catalog_service.py](../../backend/services/word_catalog_service.py), [word_detail_enrichment.py](../../backend/services/word_detail_enrichment.py) | Phase 2 |
| `ai-execution-service` | LLM execution, streaming, provider routing, prompt runtime | Prompt-run records, model routing config, tool execution logs, LLM cache | May read gateway-provided user context, catalog snippets, notes context payloads | AI execution database and provider cache | [llm_service](../../backend/services/llm_service), [ai_assistant_ask_service.py](../../backend/services/ai_assistant_ask_service.py), [ai_assistant_tool_service.py](../../backend/services/ai_assistant_tool_service.py) | Phase 2 |
| `learning-core-service` | Learning facts, quick-memory, wrong words, sessions, stats source-of-truth | Study sessions, quick-memory records, wrong words, chapter progress, learner-profile source facts | May read catalog IDs and AI summaries through explicit interfaces | Learning-state database and derived read models | [study_sessions.py](../../backend/services/study_sessions.py), [session_logging_service.py](../../backend/services/session_logging_service.py), [learning_stats_service.py](../../backend/services/learning_stats_service.py), [ai_wrong_words_service.py](../../backend/services/ai_wrong_words_service.py) | Phase 4 |
| `notes-service` | Notes, summaries, exports, journal outputs | Notes, summary jobs, exports, rendered summary records | May read learning-core facts and AI execution outputs through service APIs | Notes database and export artifact storage | [notes_query_service.py](../../backend/services/notes_query_service.py), [notes_summary_service.py](../../backend/services/notes_summary_service.py), [notes_summary_job_service.py](../../backend/services/notes_summary_job_service.py) | Phase 4 |
| `identity-service` | Identity, sessions, refresh tokens, email verification, account security | Users, revoked tokens, email verification, auth audit records | May expose identity and permission claims to gateway and internal services | Identity database and security audit store | [auth_session_service.py](../../backend/services/auth_session_service.py), [auth_repository.py](../../backend/services/auth_repository.py), [auth_email_service.py](../../backend/services/auth_email_service.py) | Phase 4 |
| `admin-ops-service` | Admin dashboards, user-management operations, audit and reporting | Admin audit records, moderation state, ops read models | May read identity, learning-core, notes, and catalog through service contracts | Admin operations database and analytics read models | [admin_overview_service.py](../../backend/services/admin_overview_service.py), [admin_user_management_service.py](../../backend/services/admin_user_management_service.py) | Phase 4 |
...
- Table-level Wave 4 audit: [service-table-boundary-audit.md](./service-table-boundary-audit.md)
- Gateway contract skeleton: [gateway-service-contracts.md](./gateway-service-contracts.md)
