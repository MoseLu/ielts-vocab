...
def _load_sync_endpoint_support():
    ...
    return build_smart_stats_response, sync_quick_memory_response, sync_smart_stats_response


@ai_bp.route('/quick-memory/sync', methods=['POST'])
@token_required
def sync_quick_memory(current_user: User):
    _, sync_quick_memory_response, _ = _load_sync_endpoint_support()
...
def get_smart_stats(current_user: User):
    build_smart_stats_response, _, _ = _load_sync_endpoint_support()
...
def sync_smart_stats(current_user: User):
    _, _, sync_smart_stats_response = _load_sync_endpoint_support()
