...
class TestExampleAudioRoute:
    def test_dashscope_example_audio_uses_provider_aware_tts(self, client, monkeypatch, tmp_path):
        ...
        assert 'X-Audio-Cache-Key' in res.headers
    ...
    def test_example_audio_supports_get_playback_urls(self, client, monkeypatch, tmp_path):
        monkeypatch.setattr(tts, '_cache_dir', lambda: tmp_path)
        monkeypatch.setattr(tts, 'default_cache_identity', lambda: ('qwen-tts-2025-05-22', 'Cherry'))
        monkeypatch.setenv('BAILIAN_TTS_PROVIDER', 'dashscope')
        ...
        monkeypatch.setattr(
            ...
            lambda text, model, voice: VALID_MP3,
        )

        res = client.get('/api/tts/example-audio?sentence=Hello%2C%20world%21&word=hello')
    ...
    def test_example_audio_ignores_range_requests(self, client, monkeypatch, tmp_path):
        monkeypatch.setattr(tts, '_cache_dir', lambda: tmp_path)
        monkeypatch.setattr(tts, 'default_cache_identity', lambda: ('qwen-tts-2025-05-22', 'Cherry'))
        monkeypatch.setenv('BAILIAN_TTS_PROVIDER', 'dashscope')
        ...
        res = client.get(
            '/api/tts/example-audio?sentence=Hello%2C%20world%21&word=hello',
    ...
    def test_example_audio_metadata_only_returns_cached_size(self, client, monkeypatch, tmp_path):
        ...
        assert res.headers['X-Audio-Bytes'] == str(len(VALID_MP3))
        assert res.headers['X-Audio-Cache-Key'].startswith(f'{cache_key}:')
        assert not res.data
