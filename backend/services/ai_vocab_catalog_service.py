...
def _load_lightweight_book_vocabulary(book_id: str) -> list[dict]:
    book = books_registry_service.get_vocab_book(book_id) or {}
    file_name = str(book.get('file') or '').strip()
    if not file_name:
        return []
    file_path = Path(books_vocabulary_loader_service.get_vocab_data_path()) / file_name
    try:
...
def _build_book_quick_memory_vocab_lookup_once(book_id: str) -> dict[str, list[dict]]:
    ...
    for word in (
        ...
        or books_catalog_service.load_book_vocabulary(book_id)
        or []
    ):
        word_text = (word.get('word') or '').strip()
        if not word_text:
...
def _get_quick_memory_vocab_entries(word_key: str, *, book_id: str | None = None) -> list[dict]:
    if book_id:
        return [
            dict(entry)
            for entry in (_get_book_quick_memory_vocab_lookup(book_id).get(word_key) or [])
        ]
    return [dict(entry) for entry in (_get_quick_memory_vocab_lookup().get(word_key) or [])]
...
def _resolve_unique_quick_memory_vocab_context(word_key: str) -> tuple[str | None, str | None] | None:
    contexts = {
        (
            (entry.get('book_id') or '').strip() or None,
            normalize_chapter_id(entry.get('chapter_id')),
        )
        for entry in _get_quick_memory_vocab_entries(word_key)
...
def _resolve_quick_memory_vocab_entry(
    ...
    *,
    ...
    entries = _get_quick_memory_vocab_entries(word_key, book_id=book_id) if book_id else (
        _get_quick_memory_vocab_entries(word_key)
    )
    ...
    if book_id and not entries:
        entries = _get_quick_memory_vocab_entries(word_key)
