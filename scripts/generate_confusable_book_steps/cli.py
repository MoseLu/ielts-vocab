def main() -> None:
    ...
    high_value_listening_presets = build_high_value_listening_presets(
    ...
    high_value_candidate_lengths = [
        len(candidates) for candidates in high_value_listening_presets.values()
    ]
    ...
    HIGH_VALUE_PRESET_OUTPUT_PATH.write_text(
        json.dumps({
            "candidate_limit": LISTENING_PRESET_CANDIDATE_LIMIT,
            "minimum_candidates": HIGH_VALUE_PRESET_TARGET_MIN_CANDIDATES,
            "sources": LISTENING_PRESET_SOURCE_FILES,
            "priority_group_source": PRIORITY_GROUPS_PATH.name,
            "covered_words": len(high_value_listening_presets),
            "min_candidates_per_word": min(high_value_candidate_lengths) if high_value_candidate_lengths else 0,
            "max_candidates_per_word": max(high_value_candidate_lengths) if high_value_candidate_lengths else 0,
            "words": high_value_listening_presets,
        }, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    ...
    print(f"Wrote {HIGH_VALUE_PRESET_OUTPUT_PATH}")
    print(
    ...
    print(
        f"High-value presets: {len(high_value_listening_presets)} words "
        f"| min candidates: {min(high_value_candidate_lengths) if high_value_candidate_lengths else 0} "
        f"| max candidates: {max(high_value_candidate_lengths) if high_value_candidate_lengths else 0}"
    )
