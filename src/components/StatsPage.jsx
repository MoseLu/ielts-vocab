...
function StatsPage({ user }) {
  ...
  const todayWords = Object.values(dayProgress).reduce((sum, d) => {
  ...
  const chartData = Array.from({ length: 30 }, (_, i) => {
    ...
    const learned = fromApi ? fromApi.correct_count : (fromLocal ? fromLocal.correctCount : 0)
  ...
  const allBookProgress = (() => {
    try { return JSON.parse(localStorage.getItem('all_book_progress') || '{}') } catch { return {} }
  })()

  const chapterStats = Object.entries(allBookProgress).map(([bookId, data]) => ({
    bookId,
