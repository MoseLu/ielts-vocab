...
For this project, the browser-facing production-style entry is wired through a local reverse-proxy chain:

1. Frontend dev server runs locally on `http://127.0.0.1:3002`
2. Local `nginx` listens on port `80` and proxies requests to `3002`
3. `natapp` exposes `https://axiomaticworld.com` and tunnels that traffic to local port `80`
4. As a result, requests hitting `https://axiomaticworld.com` are expected to flow through `natapp -> local :80 -> nginx -> local :3002`

When debugging API, SSL, or route issues involving `https://axiomaticworld.com`, treat this proxy chain as the default deployment context. In particular, `ERR_SSL_PROTOCOL_ERROR`, broken `/api/...` requests, or mixed frontend/backend origin behavior may be caused by the tunnel or proxy layers, not only by frontend code.
