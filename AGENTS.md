...
## Current Focus
- Keep the post-Wave-5 microservice baseline stable: local/remote Redis and RabbitMQ, worker-aware release, event-driven admin/notes/ai read models, marker-driven cutover, strict split-runtime read boundaries, and Redis-backed auth/speech/search-cache workloads are all now part of the canonical runtime and should be preserved during follow-up cleanup work.
