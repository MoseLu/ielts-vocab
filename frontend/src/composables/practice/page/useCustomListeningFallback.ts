...
import type { PracticeMode, ToastType } from '../../../features/practice/types'
import { supportsListeningFallbackBookId } from '../../../features/practice/practiceSessionHelpers'
...
export function useCustomListeningFallback({
  ...
  const supportsListeningFallback = supportsListeningFallbackBookId(resolvedPracticeBookId ?? bookId)
  const practiceMode: PracticeMode = requestedPracticeMode === 'listening' && fallbackScope === practiceScopeKey
    ? 'meaning'
    : requestedPracticeMode

  useEffect(() => {
    if (requestedPracticeMode !== 'listening' || !supportsListeningFallback) {
      noticeScopeRef.current = null
    }
  }, [requestedPracticeMode, supportsListeningFallback])
  ...
  const handleCustomListeningFallback = useCallback(() => {
    ...
    if (noticeScopeRef.current !== practiceScopeKey) {
      ...
      showToast?.('当前词表没有听音题素材，已自动切换到词义模式。', 'info')
    }
    onModeChange?.('meaning')
  }, [onModeChange, practiceScopeKey, showToast])

  return {
    isCustomPracticeScope: supportsListeningFallback,
    practiceMode,
