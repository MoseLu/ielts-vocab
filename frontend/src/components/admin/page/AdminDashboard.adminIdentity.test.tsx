...
describe('AdminDashboard admin identity', () => {
  ...
  it('falls back to the shared default avatar for the current admin when no custom avatar exists', async () => {
    useAuthMock.mockReturnValue({
    ...
    await waitFor(() => {
      expect(container.querySelector('.admin-avatar')).not.toBeNull()
    })

    const rowAvatar = container.querySelector('.admin-avatar') as HTMLImageElement | null
    expect(rowAvatar?.getAttribute('src')).toBe('/default-avatar.jpg')

    fireEvent.click(await screen.findByText('admin'))

    await waitFor(() => {
      expect(container.querySelector('.admin-modal-avatar')).not.toBeNull()
    })

    const modalAvatar = container.querySelector('.admin-modal-avatar') as HTMLImageElement | null
    expect(modalAvatar?.getAttribute('src')).toBe('/default-avatar.jpg')
  })
})
