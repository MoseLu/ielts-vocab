...
def notes_bootstrap_marker_name(projection_name: str) -> str:
    return f'{projection_name}.bootstrap'


def notes_projection_bootstrap_ready(projection_name: str) -> bool:
    marker = NotesProjectionCursor.query.filter_by(
        projection_name=notes_bootstrap_marker_name(projection_name)
    ).first()
    return bool(
        marker is not None
...
def _touch_projection_cursor(projection_name: str, *, session, bootstrap_started_at: datetime) -> None:
    ...
    for cursor_name in (
        projection_name,
        notes_bootstrap_marker_name(projection_name),
    ):
        cursor = session.query(NotesProjectionCursor).filter_by(projection_name=cursor_name).first()
        if cursor is None:
            cursor = NotesProjectionCursor(projection_name=cursor_name)
            session.add(cursor)
...
def bootstrap_notes_projection_snapshots(*, session=None) -> dict[str, int]:
    session = session or _query_session(NotesProjectionCursor)
