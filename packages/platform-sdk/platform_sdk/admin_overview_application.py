...
def get_effective_book_progress(user_id: int) -> list[dict]:
    ...
    effective_rows = []
    for book_id in set(progress_by_book) | set(chapters_by_book):
...
def _build_overview_response() -> tuple[dict, int]:
    ...
    daily_rows = admin_overview_repository.list_daily_activity_rows(
...
def _build_users_response(
    *,
    ...
    return {
        'users': summaries[start:start + per_page],
        'total': total,
        'page': page,
        'per_page': per_page,
