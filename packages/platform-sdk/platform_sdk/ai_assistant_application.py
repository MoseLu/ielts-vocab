...
from platform_sdk.ai_custom_books_application import (
...
from platform_sdk.ai_prompt_support import build_context_msg, parse_options, strip_options
from platform_sdk.ai_related_notes_support import (
...
def build_ask_messages(current_user, user_message: str, frontend_context: dict) -> list[dict]:
    ...
    try:
        ctx_data = build_context_payload(current_user.id)
        ...
        if frontend_context:
            messages.append({
                'role': 'user',
                'content': f'[当前学习状态]\n{build_context_msg(frontend_context)}',
            })
...
def ask_stream_api_response(current_user, body: dict | None):
    body = body or {}
    user_message = str(body.get('message', '') or '').strip()
    frontend_context = body.get('context') if isinstance(body.get('context'), dict) else {}
...
def generate_book_api_response(current_user, body: dict | None):
    return generate_book_response(current_user, body or {})


def list_custom_books_api_response(current_user):
    return list_custom_books_response(current_user)


def get_custom_book_api_response(current_user, book_id: str):
