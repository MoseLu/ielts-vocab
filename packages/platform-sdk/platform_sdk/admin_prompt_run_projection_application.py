...
PROMPT_RUN_PROJECTION = 'admin.prompt-run-directory'
PROMPT_RUN_QUEUE = f'{ADMIN_OPS_SERVICE_NAME}.{AI_PROMPT_RUN_COMPLETED_TOPIC}'
...
def _projection_cursor(session):
    ...
    if cursor is None:
        cursor = AdminProjectionCursor(projection_name=PROMPT_RUN_PROJECTION)
        session.add(cursor)
    return cursor


def upsert_projected_prompt_run(payload: dict, *, event_id: str, session=None) -> AdminProjectedPromptRun:
    session = session or _query_session(AdminProjectedPromptRun)
    ...
    record = session.get(AdminProjectedPromptRun, prompt_run_id)
    if record is None:
        record = AdminProjectedPromptRun(
            id=prompt_run_id,
            event_id=event_id,
...
def consume_ai_prompt_run_completed_event(
    *,
    event_id: str,
    ...
    session=None,
) -> tuple[AdminProjectedPromptRun, bool]:
    session = session or _query_session(AdminProjectedPromptRun)
    ...
    if inbox_record.status == 'processed':
        existing = session.get(AdminProjectedPromptRun, int(payload.get('prompt_run_id') or 0))
        if existing is None:
            raise RuntimeError('processed inbox event is missing projected prompt run state')
        return existing, False

    begin_inbox_processing(inbox_record)
    try:
        projected_prompt_run = upsert_projected_prompt_run(payload, event_id=event_id, session=session)
        cursor = _projection_cursor(session)
        cursor.last_event_id = event_id
        cursor.last_topic = AI_PROMPT_RUN_COMPLETED_TOPIC
...
def drain_ai_prompt_run_completed_queue(*, limit: int = 50, connection=None) -> int:
    ...
    channel.queue_declare(queue=PROMPT_RUN_QUEUE, durable=True)
    channel.queue_bind(
        exchange=exchange_name,
        queue=PROMPT_RUN_QUEUE,
    ...
    try:
        while processed < max(1, limit):
            method_frame, properties, body = channel.basic_get(queue=PROMPT_RUN_QUEUE, auto_ack=False)
            ...
            try:
                ...
                consume_ai_prompt_run_completed_event(
                    event_id=event_id,
                    producer_service=producer_service,
