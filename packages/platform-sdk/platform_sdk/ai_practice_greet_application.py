...
def greet_response(current_user, body: dict | None):
    body = body or {}
    ...
    try:
        ...
        messages.append({'role': 'user', 'content': '补充要求：默认自然开场，不要默认输出选项。'})
...
def correct_text_response(current_user, body: dict | None):
    body = body or {}
    ...
    track_metric(current_user.id, 'writing_correction_used', {'length': len(text_in.split())})
    return jsonify(result), 200


def correction_feedback_response(current_user, body: dict | None):
    adopted = bool((body or {}).get('adopted'))
    track_metric(current_user.id, 'writing_correction_adoption', {'adopted': adopted})
